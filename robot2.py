...
if not exists("./config.json"):
    ...
    smtps = {
        ...
        "@yahoo.com.cn": "server = smtp.mail.yahoo.com.cn, port = 995",
...
def report(data: dict, r: str):
    print("违规用户UID：", data["mid"])
    print("违规类型：色情")
    print("违规信息发布形式， (评论)")
    print("详细内容： ", json.dumps(data, ensure_ascii=False, indent=4))
    print("\n规则： ", r, "\n")
...
def setMethod():
    ...
    for key, value in method_choices.items():
        print(f"{key}: {value}")
    method = input("请选择操作方式： ")
